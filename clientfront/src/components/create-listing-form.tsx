  ...
  async function submit(forceCreate = false) {
    ...
    try {
      ...
      if (error instanceof ClientApiError && error.status === 409) {
        const nextSuggestions = (error.details as { meta?: { suggestions?: ProductSuggestion[] } } | undefined)?.meta?.suggestions ?? [];
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-[1.75rem] border border-stone-900/10 bg-stone-50 p-5">
        <p className="text-xs uppercase tracking-[0.25em] text-stone-500">Suggestions</p>
        <h2 className="mt-2 text-xl font-semibold text-stone-950">Reuse global products</h2>
              ...
              <button
                ...
                className={`block w-full rounded-2xl border px-4 py-4 text-left transition ${
                  reuseProductId === suggestion.product_id
                    ...
                    : "border-stone-200 bg-white text-stone-900 hover:border-stone-900"
                }`}
              >
      ...
      <section className="rounded-[1.75rem] border border-stone-900/10 bg-white p-5">
        <p className="text-xs uppercase tracking-[0.25em] text-stone-500">Create listing</p>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-stone-950">Publish inventory to this tenant</h1>
