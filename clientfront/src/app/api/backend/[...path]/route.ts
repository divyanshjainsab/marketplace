...
import { APP_JWT_COOKIE, APP_REFRESH_COOKIE } from "@/lib/auth";
...
function tenantHeaderValue() {
  return cookies().get("cf_tenant")?.value ?? process.env.NEXT_PUBLIC_DEFAULT_TENANT ?? "";
}
...
async function handle(req: NextRequest, params: { path: string[] }) {
  ...
  let accessToken = cookies().get(APP_JWT_COOKIE)?.value ?? null;
  let refreshToken = cookies().get(APP_REFRESH_COOKIE)?.value ?? null;
  ...
  if (refreshedTokens) {
    response.cookies.set(APP_JWT_COOKIE, refreshedTokens.token, {
      httpOnly: true,
      sameSite: "lax",
    ...
    response.cookies.set(APP_REFRESH_COOKIE, refreshedTokens.refresh_token, {
      httpOnly: true,
      sameSite: "lax",
      secure: process.env.NODE_ENV === "production",
      path: "/",
    ...
    response.cookies.set(APP_JWT_COOKIE, "", {
      httpOnly: true,
      sameSite: "lax",
      secure: process.env.NODE_ENV === "production",
      path: "/",
    ...
    response.cookies.set(APP_REFRESH_COOKIE, "", {
      httpOnly: true,
      sameSite: "lax",
      secure: process.env.NODE_ENV === "production",
      path: "/",
