...
export async function POST(req: Request) {
  const body = (await req.json().catch(() => null)) as
    | { token?: string; refresh_token?: string; exp?: number; refresh_exp?: number }
    | null;
  ...
  const refreshMaxAge = body?.refresh_exp
    ? Math.max(body.refresh_exp - Math.floor(Date.now() / 1000), 300)
    : undefined;
